        ...
        <b-navbar id="main-menu" toggleable="lg" type="dark" variant="dark">
            ...
            <b-collapse id="main-menu-collapse" is-nav>
                ...
                <b-navbar-nav class="ml-auto">
                    <b-nav-item id="dataset-filename" disabled>
...
<script>
    ...
    class Dataset {
        constructor() {
            const xmlDataset = new DOMParser().parseFromString(getXML(), 'text/xml');
    ...
    class Entry {
         ...
         */
        isEmpty() {
            return this.isChild() ? this.data === null : this.data.length == 0;
        }
        /**
         * @returns {!Boolean}
         */
         ...
         */
        get() {
            return this.isChild() ? this.data !== null ? this.data.get() : '' : this.data;
        }
        /**
         * @returns {?Array.<!Entry>}
         */
         ...
         */
        set(data) {
            if (this.isChild() && typeof data === 'string') {
    ...
    class Station {
        ...
        constructor(station, vue) {
            ...
            /** @type {!Function} */
            this.transferItems = () => this.dataset.get('transfers').records.filter(record => this.tree.contains(record['from_stop_id'].get()) || this.tree.contains(record['to_stop_id'].get()));
            ...
            /** @type {!Function} */
            this.pathwayItems = () => this.dataset.get('pathways').records.filter(record => this.tree.contains(record['from_stop_id'].get()) || this.tree.contains(record['to_stop_id'].get()));
        ...
        afterConstructed() {
            ...
            this.mockups.find(mockup => mockup.title === 'Transfers').table.push({
                key: 'transfers-row-1',
                data: [
                    { action: { icon: null, text: null, special: 'stops.transfers' }, colspan: 2, rowspan: 1 }
                ]
            });
            const createTransferFrom = {
                callback: () => {
                    this.dataset.get('transfers').createRecord([ { property: 'from_stop_id', value: this.record['stop_id'].get() } ]);
                    this.transfersKey += 1;
                },
                icon: null,
                text: 'New (From)'
            };
            const createTransferTo = {
                callback: () => {
                    this.dataset.get('transfers').createRecord([ { property: 'to_stop_id', value: this.record['stop_id'].get() } ]);
                    this.transfersKey += 1;
                },
                icon: null,
                text: 'New (To)'
            ...
            this.mockups.find(mockup => mockup.title === 'Transfers').table.push({
                key: 'transfers-row-2',
                data: [
                    { action: createTransferFrom, colspan: 1, entry: null, label: null, rowspan: 1 },
                    { action: createTransferTo, colspan: 1, entry: null, label: null, rowspan: 1 }
                ]
            });
            ...
            this.mockups.find(mockup => mockup.title === 'Pathways').table.push({
                key: 'pathways-row-1',
                data: [
                    { action: { icon: null, text: null, special: 'stops.pathways' }, colspan: 2, rowspan: 1 }
                ]
            });
            const createPathwayFrom = {
                callback: () => {
                    this.dataset.get('pathways').createRecord([ { property: 'from_stop_id', value: this.record['stop_id'].get() } ]);
                    this.pathwaysKey += 1;
                },
                icon: null,
                text: 'New (From)'
            };
            const createPathwayTo = {
                callback: () => {
                    this.dataset.get('pathways').createRecord([ { property: 'to_stop_id', value: this.record['stop_id'].get() } ]);
                    this.pathwaysKey += 1;
                },
                icon: null,
                text: 'New (To)'
            ...
            this.mockups.find(mockup => mockup.title === 'Pathways').table.push({
                key: 'pathways-row-2',
                data: [
                    { action: createPathwayFrom, colspan: 1, entry: null, label: null, rowspan: 1 },
                    { action: createPathwayTo, colspan: 1, entry: null, label: null, rowspan: 1 }
                ]
            });
    ...
    class Trip {
        ...
        constructor(trip, vue) {
            ...
            /** @type {!Function} */
            this.stopItems = () => this.record['trip_id'].children.filter(child => child.field.file.identifier === 'stop_times').map(entry => entry.record).sort((a, b) => a['stop_sequence'].get() < b['stop_sequence'].get() ? 0 : 1);
            ...
            /** @type {!Function} */
            this.frequencyItems = () => this.record['trip_id'].children.filter(child => child.field.file.identifier === 'frequencies').map(entry => entry.record);
        ...
        afterConstructed() {
            ...
            const createStop = {
                callback: () => {
                    const stopTimes = this.dataset.get('stop_times');
                    const tripID = this.record['trip_id'].get();
                    const stop_sequence = stopTimes.records.reduce((maximum, record) => {
                        const value = parseInt(record['stop_sequence'].get(), 10);
                        return record['trip_id'].get() === tripID && maximum < value ? value : maximum;
                    }, 0);
                    stopTimes.createRecord([
                        { property: 'trip_id', value: tripID },
                        { property: 'stop_sequence', value: (stop_sequence + 1).toString() }
                    ]);
                    this.stopsKey += 1;
                },
                icon: null,
            ...
            this.mockups.find(mockup => mockup.title === 'Stops').table.push({
                key: 'stops-row-2',
                data: [
                    { action: createStop, colspan: 1, entry: null, label: null, rowspan: 1 }
                ]
            });
            ...
            const createFrequency = {
                callback: () => {
                    this.dataset.get('frequencies').createRecord([ { property: 'trip_id', value: this.record['trip_id'].get() } ]);
                    this.frequenciesKey += 1;
                },
                icon: null,
                text: 'New'
            };
            this.mockups.find(mockup => mockup.title === 'Frequencies').table.push({
                key: 'frequencies-row-2',
                data: [
                    { action: createFrequency, colspan: 1, entry: null, label: null, rowspan: 1 }
                ]
            });
